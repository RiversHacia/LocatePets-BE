...
class Messages {
    ...
    async getMessagesByUserId(userId = 0, mailboxType = 'inbox', sort = 'DESC') {
        ...
        SELECT
            ...
            u.name,
            COUNT(m2.id) AS totalMessagesInChain,
            SUM(CASE WHEN ${mailboxType === 'inbox' ? 'm2.isReadRecipient' : 'm2.isReadSender'} = 0 THEN 1 ELSE 0 END) AS unreadMessages
        FROM ${this.#messages} m
        LEFT JOIN ${this.#messages} m2 ON m.msgChainId = m2.msgChainId
        LEFT JOIN ${this.#userInfo} u ON m.senderUserId = u.user_id
        WHERE ${whereClause}
    ...
    async getMessagesByUUID(msgChainId = '', getDeleted = false) {
        try {
            ...
            WHERE
                m.msgChainId = '${msgChainId}'
                AND ${getDeleted ? 'm.isDeletedRecipient = 1' : 'm.isDeletedRecipient = 0'}
            ORDER BY m.createdDate ASC
            `;
            const results = await this.#db.query(sql);
            return results;
    ...
    async createMessageEntry(senderUserId = 0, recipientUserId = 0, subject = '', message = '', chainId = '', metadata = {}) {
        try {
            ...
            VALUES
                (${chainId ? `'${chainId}'` : `'${uuid1}'`}, ${senderUserId}, ${recipientUserId}, 0, 0, 0, 0, '${subject}', '${message}', NOW(), '${metadata}', '${uuid2}');
            `
    ...
    async setMessageAsRead(messageId = 0, userId = 0) {
        try {
            const sql = `
            UPDATE ${this.#messages}
            SET ${userId === 'senderUserId' ? 'isReadSender' : 'isReadRecipient'} = 1
            WHERE msgId = '${messageId}'
            `;
            const results = await this.#db.query(sql);
            if (results.affectedRows === 0) {
